from __future__ import absolute_import

# from sentry.integrations import Integration, IntegrationFeatures, IntegrationProvider, IntegrationMetadata
# from sentry.integrations.atlassian_connect import AtlassianConnectValidationError, get_integration_from_request
# from sentry.integrations.migrate import PluginMigrator
# from sentry.integrations.repositories import RepositoryMixin
# from sentry.pipeline import NestedPipelineView, PipelineView
# from sentry.identity.pipeline import IdentityProviderPipeline
# from django.utils.translation import ugettext_lazy as _

# from sentry.models import Repository
# from sentry.utils.http import absolute_uri

# from .repository import GitlabRepositoryProvider
# from .client import GitlabApiClient
# from .issues import GitlabIssueBasicMixin

# DESCRIPTION = """
# With the Gitlab integration, you can:
#  * Track commits and releases (learn more [here](https://docs.sentry.io/learn/releases/))
#  * Create Gitlab issues from Sentry
#  * Link Sentry issues to existing Gitlab issues
#  * Resolve Sentry issues via Gitlab commits and pull requests by including `Fixes PROJ-ID` in the message
# """
# metadata = IntegrationMetadata(
#     description=DESCRIPTION.strip(),
#     author='The Sentry Team',
#     noun=_('Installation'),
#     issue_url='https://github.com/getsentry/sentry/issues/new?title=Gitlab%20Integration:%20&labels=Component%3A%20Integrations',
#     source_url='https://github.com/getsentry/sentry/tree/master/src/sentry/integrations/bitbucket',
#     aspects={},
# )
# scopes = (
#     'account',
#     'email',
#     'issue:write',
#     'pullrequest',
#     'repository',
#     'repository:admin',
#     'team',
#     'webhook',
# )


# class GitlabIntegration(Integration, GitlabIssueBasicMixin, RepositoryMixin):

#     def get_client(self):
#         return GitlabApiClient(
#             self.model.metadata['base_url'],
#             self.model.metadata['access_token'],
#         )

#     def get_repositories(self, query=None):
#         if not query:
#             resp = self.get_client().get_repos(self.username)
#             return [{
#                 'identifier': repo['full_name'],
#                 'name': repo['full_name'],
#             } for repo in resp.get('values', [])]

#         full_query = (u'name~"%s"' % (query)).encode('utf-8')
#         resp = self.get_client().search_repositories(self.username, full_query)
#         return [{
#             'identifier': i['full_name'],
#             'name': i['full_name']
#         } for i in resp.get('values', [])]

#     def get_unmigratable_repositories(self):
#         repos = Repository.objects.filter(
#             organization_id=self.organization_id,
#             provider='bitbucket',
#         )

#         accessible_repos = [
#             r['identifier'] for r in self.get_repositories()
#         ]

#         return filter(
#             lambda repo: repo.name not in accessible_repos,
#             repos,
#         )

#     def reinstall(self):
#         self.reinstall_repositories()


# class GitlabIntegrationProvider(IntegrationProvider):
#     key = 'gitlab'
#     name = 'Gitlab'
#     metadata = metadata
#     scopes = scopes
#     integration_cls = GitlabIntegration
#     features = frozenset([IntegrationFeatures.ISSUE_BASIC, IntegrationFeatures.COMMITS])

#     def get_pipeline_views(self):
#         identity_pipeline_config = {
#             'redirect_url': absolute_uri('/extensions/gitlab/setup/'),
#         }
#         identity_pipeline_view = NestedPipelineView(
#             bind_key='identity',
#             provider_key='gitlab',
#             pipeline_cls=IdentityProviderPipeline,
#             config=identity_pipeline_config,
#         )
#         return [identity_pipeline_view, VerifyInstallation()]

#     def post_install(self, integration, organization):
#         repos = Repository.objects.filter(
#             organization_id=organization.id,
#             provider='gitlab',
#         )

#         unmigrateable_repos = self \
#             .get_installation(integration, organization.id) \
#             .get_unmigratable_repositories()

#         for repo in filter(lambda r: r not in unmigrateable_repos, repos):
#             repo.update(integration_id=integration.id)

#         PluginMigrator(integration, organization).call()

#     def build_integration(self, state):
#         if state.get('publicKey'):
#             principal_data = state['principal']
#             return {
#                 'provider': self.key,
#                 'external_id': state['clientKey'],
#                 'name': principal_data['username'],
#                 'metadata': {
#                     'base_url': state['baseApiUrl'],
#                     'domain_name': principal_data['links']['html']['href'].replace('https://', ''),
#                     'access_token': state['access_token'],  # meh
#                 },
#             }
#         else:
#             return {
#                 'provider': self.key,
#                 'external_id': state['external_id'],
#                 'expect_exists': True,
#             }

#     def setup(self):
#         from sentry.plugins import bindings
#         bindings.add(
#             'integration-repository.provider',
#             GitlabRepositoryProvider,
#             id='integrations:%s' % self.key,
#         )
